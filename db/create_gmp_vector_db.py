# -*- coding: utf-8 -*-
"""
GMP JSONL → 임베딩 → 벡터 DB 저장 파이프라인
JSONL 파일의 청킹된 GMP 문서들을 임베딩하고 Chroma 벡터 DB에 저장
"""
import argparse
import json
import os
...
# ==================== 설정 ====================
DB_PATH = "./gmp_vector_db"
COLLECTION_NAME = "gmp_documents"
...
def prepare_gmp_documents_with_metadata(chunks: List[Dict[str, Any]]) -> List[Document]:
    """JSONL 데이터를 LangChain Document 형식으로 변환"""
    log("Document 형식으로 변환 중...")
    ...
    for chunk in chunks:
        ...
        metadata = {
            ...
            "normative_strength": chunk.get("normative_strength"),
    ...
    log(f"{len(documents)}개 Document 변환 완료")
    return documents
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GMP JSONL 파일을 임베딩하여 벡터 DB로 저장"
    )
    parser.add_argument(
        "--input", 
        type=str, 
    ...
    try:
        ...
        # 2단계: Document 형식 변환
        documents = prepare_gmp_documents_with_metadata(chunks)
